...
export function AdminShell({
  ...
  return (
    <div className="min-h-screen bg-calm-50 lg:grid lg:grid-cols-[260px_1fr]">
      <aside className="border-b border-calm-200 bg-white lg:min-h-screen lg:border-b-0 lg:border-r">
        <div className="flex items-center justify-between gap-3 px-4 py-4 lg:block lg:px-5">
          <Link className="flex items-center gap-3" href="/admin/dashboard">
            ...
            <span>
              <span className="block text-sm font-black text-calm-900">LeadTask Admin</span>
        ...
        <nav className="flex gap-2 overflow-x-auto px-4 pb-4 lg:block lg:space-y-1 lg:px-3 lg:pt-2">
          {links.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                className={cn(
                  "focus-ring flex min-w-max items-center gap-3 rounded-md px-3 py-2.5 text-sm font-bold transition",
                  active
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-20 hidden border-b border-calm-200 bg-white/95 px-6 py-3 backdrop-blur lg:flex lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-black text-calm-900">{user.name}</p>
            <p className="text-xs font-medium text-calm-500">Full control access</p>
          </div>
